...
def get_request_session(url, data, app_session='banban-release'):
    ...
    if not url.startswith('https://'):
        url = f'https://{url}'
        get_log('get_url.log').error(f'{url} is error')
    ...
    try:
        ...
        get_log('get_url.log').error(f'RequestException url: {url}')
        get_log('get_url.log').error(e)
        return ()
    except Exception as e:
        get_log('get_url.log').error(f'Exception url: {url}')
        get_log('get_url.log').error(e)
        return ()
